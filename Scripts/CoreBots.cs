...
public class CoreBots
{
    ...
    //butler-sync start
    private CancellationTokenSource? _locationSyncCTS;
    private string? _lastWrittenLocation;
    private static int _syncWriterStarted = 0;
    //butler-sync end
    ...
    /// butler-sync start
    private void InitializeButlerSync()
    {
        // Only start the writer once per session (thread-safe)
        if (Interlocked.Exchange(ref _syncWriterStarted, 1) == 0)
        {
            ...
            bool TryGetRoomNumber(out string roomNumber)
            {
                roomNumber = "";
                try
                {
                    string fullName = Bot.Map.FullName ?? "";
                    ...
                    if (!int.TryParse(lastPart, out int parsedRoom)) return false;
            ...
            void WriteLocation(bool forceTimestampRefresh = false)
            {
                try
                {
                    string currentMap = Bot.Map.Name ?? "unknown";
                    if (!TryGetRoomNumber(out string currentRoom)) return;

                    string currentCell = Bot.Player?.Cell ?? "Enter";
                    string currentPad = Bot.Player?.Pad ?? "Left";
                    string loggedIn = Bot.Player.LoggedIn ? "1" : "0";
                    string serverIP = Bot.Player.ServerIP?.Trim() ?? "unknown";
                    bool inCombatOrTarget = Bot.Player.InCombat || Bot.Player.HasTarget;
                    string inCombat = inCombatOrTarget ? "1" : "0";
                    string hasTarget = Bot.Player.HasTarget ? "1" : "0";
                    string alive = Bot.Player.Alive ? "1" : "0";

                    string baseLocation = $"{currentMap}|{currentRoom}|{currentCell}|{currentPad}|{loggedIn}|{serverIP}|{inCombat}|{hasTarget}|{alive}";
                    bool locationChanged = _lastWrittenLocation != baseLocation;

                    if (locationChanged || forceTimestampRefresh)
                    {
                        string currentLocation = $"{currentMap}|{currentRoom}|{currentCell}|{currentPad}|{loggedIn}|{serverIP}|{DateTime.UtcNow.Ticks}|{inCombat}|{hasTarget}|{alive}";
                        string path = Path.Combine(charLocDir, $"{Bot.Player.Username}_location.sync");
                        string tempPath = path + ".tmp";

                        using (FileStream stream = new(tempPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
                        ...
                        _lastWrittenLocation = baseLocation;
                    }
                }
                catch { }
            }

            Bot.Events.MapChanged += (map) => WriteLocation(true);
            WriteLocation(true);

            _locationSyncCTS?.Cancel();
            _locationSyncCTS?.Dispose();
            _locationSyncCTS = new CancellationTokenSource();

            // FAST LOOP
            Task.Run(async () =>
            {
                var token = _locationSyncCTS.Token;
                string? lastRoom = null, lastCell = null, lastPad = null;
                bool lastLoggedIn = false, lastInCombat = false, lastHasTarget = false, lastAlive = false;
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        if (!TryGetRoomNumber(out string currentRoom)) continue;
                        string currentCell = Bot.Player?.Cell ?? "Enter";
                        string currentPad = Bot.Player?.Pad ?? "Left";
                        bool currentLoggedIn = Bot.Player.LoggedIn;
                        bool currentInCombat = Bot.Player.InCombat || Bot.Player.HasTarget;
                        bool currentHasTarget = Bot.Player.HasTarget;
                        bool currentAlive = Bot.Player.Alive;
                        string currentServerIP = Bot.Player.ServerIP?.Trim() ?? "unknown";

                        bool changed = currentRoom != lastRoom ||
                                    currentCell != lastCell ||
                                    currentPad != lastPad ||
                                    currentLoggedIn != lastLoggedIn ||
                                    currentInCombat != lastInCombat ||
                                    currentHasTarget != lastHasTarget ||
                                    currentAlive != lastAlive ||
                                    !string.Equals(currentServerIP, lastServerIP, StringComparison.OrdinalIgnoreCase);
                        ...
                        if (changed)
                        {
                            ...
                            lastPad = currentPad;
                            lastLoggedIn = currentLoggedIn;
                            lastInCombat = currentInCombat;
            ...
            Task.Run(async () =>
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    try { await Task.Delay(5000, token); WriteLocation(true); }
                    catch (TaskCanceledException) { break; }
                    catch { }
                }
            }, _locationSyncCTS.Token);
    ...
    public void SetOptions(bool changeTo = true, bool disableClassSwap = false)
    {
        ...
        if (changeTo)
        {
            ...
            /// butler-sync start
            if (!loadedBot.Contains("Butler"))
            {
                InitializeButlerSync();
            }
            /// butler-sync end
